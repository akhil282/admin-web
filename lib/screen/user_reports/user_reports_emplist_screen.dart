...
import 'package:cater_admin_web/screen/user_reports/view_employee_report_screen.dart';
...
class _UserReportsScreenState extends State<UserReportsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      employees = employees.where((employee) {
        ...
        String empId = employee['empId'].toString().toLowerCase();
        return name.contains(searchValue.toLowerCase()) ||
            empId.contains(searchValue.toLowerCase());
      }).toList();
    }
  ...
  itemBuilder: (context, index) {
    // give only search value to the employees
        ...
        return GestureDetector(
          onTap: () async {
            await Navigator.push(context, MaterialPageRoute(
              builder: (context) => ViewEmployeeReportScreen(
          ...
          children: [
                 Row(children: [
            Expanded(child:  text16("${employee['userName'].toString() ?? '-'}",overflow: TextOverflow.ellipsis,maxLines: 1,color: themeColor.black,fontWeight: FontWeight.w900,),
            ),
